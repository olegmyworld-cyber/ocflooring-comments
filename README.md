...
### Tile installation city pages — 30-city rollout (2026-08-26)

Built the "Tile Installation Bellevue" Claude Design (from the uploaded
`Carpet_mobile_showroom_service_design.zip`) as a native Webflow page and
rolled it out to the same 30 cities as the carpet rollout. Everything lives
under [`webflow-scripts/tile-rollout/`](webflow-scripts/tile-rollout/).

- **Master page** `/city-of-bellevue/tile-installation-in-bellevue-wa`
  (page id `6a8f63898db417f9f6632e49`): all design sections rebuilt as native
  crawlable elements with `ti-*` classes via the WHTML builder — breadcrumbs,
  hero (with 24 hr flood-test / 2 yr warranty / 1,000+ floors stat strip),
  dark trust bar, cost section with four price-band cards, the 7-layer shower
  assembly with "Why it matters" panel and membrane proof photo, six scope
  cards with starting prices, the materials table (porcelain / ceramic /
  stone / glass / zellige), the tile cost calculator shell, day-by-day
  schedule, 5-photo gallery, three Google review quotes, an 11-question FAQ
  accordion, neighborhoods chip row, a "Tile guides" blog section, and the
  Calendly booking CTA. Shared Navbar + Global Styles components on top
  (carpet-page prop overrides applied), shared Areas slider + the design's own
  footer at the bottom. Seven tile photos extracted from the design's image
  slots and uploaded as assets (`tile-hero`, `tile-membrane`, `tile-work-1..5`),
  bound to managed Image elements.
- **Photo-quote widget**: the carpet `#ocpq` embed adapted to tile wording and
  the tile palette ([`webflow-scripts/tile-ocpq-embed-1.0.0.html`](webflow-scripts/tile-ocpq-embed-1.0.0.html)) —
  markup server-rendered in an HTML embed for crawlers; styles + logic ride in
  the bundle. Same formsubmit.co submission path and subject format as the
  carpet/repair pages.
- **Bundle** [`webflow-scripts/oc-tile-bellevue-1.0.0.js`](webflow-scripts/oc-tile-bellevue-1.0.0.js)
  (Webflow asset `6a8f66b0b69a6077af6800f4`, loaded by registered inline
  script `octilebellevue`): navbar clearance, font/hover/media CSS the
  Designer API can't express, the interactive tile estimator rendered into
  `#ti-estimator` (design rates: labor $11/sf, demo $3.50/sf, heated $13/sf,
  waterproofing $9/sf, setting $1.90/sf, 12% waste, format ×1.00/×1.28/×1.55,
  tiers $3/$7/$14, minimums $3,200/$900/$1,100, high = low × 1.22), the FAQ
  accordion, Calendly injection into `#ti-cal-mount`, the ocpq widget logic,
  and promotion of the Tile Installation slide in the shared areas slider.
  Registered inline script `tilebellevuefontscanonical` adds Instrument
  Serif + Archivo and the canonical link (page header).
- **30 pages**: the master was duplicated into every carpet city folder
  (slugs `tile-installation-in-<city>-wa`; page ids in
  [`tile-rollout/tile-pageids.json`](webflow-scripts/tile-rollout/tile-pageids.json)),
  scripts applied to each, element ids preserved so one rewrite sheet
  ([`tile-rollout/rewrite-map.json`](webflow-scripts/tile-rollout/rewrite-map.json),
  74 fields) addresses every page. City-variable div texts were converted to
  real Paragraph elements first (the headless builder downgrades TextBlocks
  to divs, which reject `set_text`).
- **Unique per-city content**: 29 packs
  ([`tile-rollout/packs/`](webflow-scripts/tile-rollout/packs/)) composed per
  city angle ([`tile-angles.json`](webflow-scripts/tile-rollout/tile-angles.json)),
  each rewriting ~56 prose fields plus SEO/OG, chips (real neighborhoods from
  the carpet packs), image alts and the carpet-page cross-link. The 8-gram
  cross-city duplicate audit
  ([`audit-packs.py`](webflow-scripts/tile-rollout/audit-packs.py)) went from
  1,325 colliding pairs after composition to **0 collisions, 0 missing facts,
  0 empty fields** after the patch cycle; every business number survives
  verbatim. Per-city JSON-LD (business + Service with city geo +
  BreadcrumbList + the city's 11 FAQ rewrites) is generated by
  [`build-jsonld.py`](webflow-scripts/tile-rollout/build-jsonld.py).
- **Site-wide**: "Tile Installation Service Areas" slide (30 city links)
  added to the shared Proudly Serving areas slider, and a "Tile Installation"
  item added to the Navbar services dropdown, both verified persisted.
