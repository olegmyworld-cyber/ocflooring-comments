...
`data_pages_tool` → `create_page`: site_id, `title` = "Carpet Installation in {CityName}, WA", `slug` = "carpet-installation-in-{citySlug}-wa", `parentFolderId` from table, `duplicateOf` = template pageId, `draft` = true. Record the returned page `id` = PAGEID.
If create_page FAILS because the slug already exists (a previous partial run), instead find the existing page: `list_pages` (limit 100, paginate) and locate the page whose slug matches — use its id as PAGEID and continue with the remaining steps (they are idempotent enough: set_text overwrites; for step 4 first QUERY ci-chip2 elements — if 29 Link-type chips already exist, skip creating; remove only Block-type leftovers; for step 6, first query for style `cg-sec` on the page — if present, skip the insert). (Element ids inside the duplicate are IDENTICAL to the template's, so the ids below all work with component=PAGEID.)
